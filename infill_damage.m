...
% 2D topopt: infill_damage(240,30,[2],300, 1)
% Regular topopt: infill_damage(240,30,[1],300, 0)

function infill_damage(nelx,nely,mdof,nloop,twodim)
%mdof[1,2]
% 1 total volume
% 2 upper bound
...
vol_max = 0.6;
p = 16;         % pNorm
r_hat = 8;      % pNorm radius
if twodim == 1
    rmin = 4.5;     % density filter radius
    penal = 1;      % stiffness penalty
else
    %volfrac = 0.5;
    %vol_max = 0.5;
    rmin = 5;
...
iLoad = 4;
Fsparse1 = sparse(2*(nely+1)*(nelx+1),1);
Fsparse2 = sparse(2*(nely+1)*(nelx+1),1);
Fsparse3 = sparse(2*(nely+1)*(nelx+1),1);
Fsparse4 = sparse(2*(nely+1)*(nelx+1),1);
Fsparse5 = sparse(2*(nely+1)*(nelx+1),1);
...
elseif iLoad == 4
    Fsparse1(2*(nelx/4)*(nely + 1) + 2, 1) = -1;
    Fsparse2(2*(nelx*3/4)*(nely + 1) + 2, 1) = -1;
    Fsparse3(2, 1) = -1;
    Fsparse4(2*(nelx)*(nely+1)+2, 1) = -1;
    Fsparse5(2*(nelx/2)*(nely + 1) + 2, 1) = -1;
    fixeddofs = union([2*(nely+1)*(nelx/10+1)-1:2*(nely+1)*(nelx/10+1)], [2*(nely+1)*(nelx*9/10+1)-1:2*(nely+1)*(nelx*9/10+1)]);
end
U1 = zeros(2*(nely+1)*(nelx+1),1);
U2 = zeros(2*(nely+1)*(nelx+1),1);
U3 = zeros(2*(nely+1)*(nelx+1),1);
U4 = zeros(2*(nely+1)*(nelx+1),1);
U5 = zeros(2*(nely+1)*(nelx+1),1);
...
while change > 0.0001 && loop < nloop
    ...
    U1(freedofs) = K(freedofs,freedofs)\Fsparse1(freedofs);
    U2(freedofs) = K(freedofs,freedofs)\Fsparse2(freedofs);
    U3(freedofs) = K(freedofs,freedofs)\Fsparse3(freedofs);
    U4(freedofs) = K(freedofs,freedofs)\Fsparse4(freedofs);
    U5(freedofs) = K(freedofs,freedofs)\Fsparse5(freedofs);


    %% OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
    ce = reshape(sum((U1(edofMat)*KE).*U1(edofMat),2),nely,nelx)+reshape(sum((U2(edofMat)*KE).*U2(edofMat),2),nely,nelx)+reshape(sum((U3(edofMat)*KE).*U3(edofMat),2),nely,nelx)+reshape(sum((U4(edofMat)*KE).*U4(edofMat),2),nely,nelx)+reshape(sum((U5(edofMat)*KE).*U5(edofMat),2),nely,nelx);
    ...
    xTilde(:) = (H*xnew(:))./Hs;
    xPhys = (tanh(beta*eta) + tanh(beta*(xTilde-eta))) / (tanh(beta*eta) + tanh(beta*(1-eta)));
    xPhys(nely*5/6:nely, nelx/4:nelx*3/4) = 0;
...
figure(2);
plotsize = 60;
topopt2D = zeros(nelx*plotsize, nely*plotsize);
for y = 1:nely
    for x = 1:nelx
        rad = xPhys(y,x);
        circlePixels = drawCircle(rad, plotsize);
        topopt2D(x*plotsize - (plotsize-1): x*plotsize, y*plotsize - (plotsize-1): y*plotsize) = circlePixels;
        
    end
end

colormap(gray); 
imagesc(transpose(topopt2D), [0, 1]); 
axis equal; 
axis tight; 
axis off; 
drawnow;
...
imwrite(transpose(topopt2D),'topopt2D.png');
disp(c)

xPhysBackup = xPhys; %make a copy of xPhys
xPhysBackup(1:nely*1/3, nelx*11/20:nelx*14/20) = 0;
%xPhysBackup(nely*3/6:nely*5/6, nelx*11/20:nelx*14/20) = 0;
xPhys = xPhysBackup;
...
figure(3);
plotsize = 60;
topopt2D_damage = zeros(nelx*plotsize, nely*plotsize);
for y = 1:nely
    for x = 1:nelx
        rad = xPhys(y,x);
        circlePixels = drawCircle(rad, plotsize);
        topopt2D_damage(x*plotsize - (plotsize-1): x*plotsize, y*plotsize - (plotsize-1): y*plotsize) = circlePixels;
        
    end
end

colormap(gray); 
imagesc(transpose(topopt2D_damage), [0, 1]); 
axis equal; 
axis tight; 
axis off; 
drawnow;

imwrite(transpose(topopt2D_damage),'topopt2D_damage.png');
% Publication
